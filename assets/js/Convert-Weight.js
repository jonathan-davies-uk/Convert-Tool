...
/**
 * Converts an weight value from one unit to another using WEIGHT_MULTIPLES.
 * @param {number} value - The numeric value to convert.
 * @param {string} fromUnit - The unit to convert from (must match a key in WEIGHT_MULTIPLES).
 * @param {string} toUnit - The unit to convert to (must match a key in WEIGHT_MULTIPLES).
 * @returns {number|null} - The converted value, or null if units are invalid.
 */
function convertWeight(value, fromUnit, toUnit) {
    if (!(fromUnit in WEIGHT_MULTIPLES) || !(toUnit in WEIGHT_MULTIPLES)) {
        return null;
    }
    // Convert from the source unit to square metres, then to the target unit
    const valueInSqMetres = value * WEIGHT_MULTIPLES[fromUnit];
    return valueInSqMetres / WEIGHT_MULTIPLES[toUnit];
}

const result = convertWeight(100, "Square metre (sq m)", "Square foot (sq ft)");
console.log(result);